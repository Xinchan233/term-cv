...
rtmpUrl = "rtmp://39.100.106.24:1935/stream/volunteer"
                ...
                if interact_timing == 0:  # just start timing
                    ...
                    if difference < interact_limit_time:
                        ...
                        if (flag == 1):
                            ...
                            try:
                                cursor.execute(
                                    "insert into event_info(event_date,event_type,event_desc,event_location,oldperson_name,url) value(now(),1,'老人正在与义工交互','房间桌子',%s,%s)" , (
                                        name, url))
                                print('Successful')
                                db.commit()
                            except:
