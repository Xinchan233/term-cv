            ...
            if actual_distance < ACTUAL_DISTANCE_LIMIT:
                ...
                if interact_timing == 0:  # just start timing
                    ...
                    if difference < interact_limit_time:
                        print('[INFO] %s, 房间桌子, %s仅交互了 %.1f 秒. 忽略' % (current_time, id_card_to_name[old_people_name[j_index]], difference))
                    else:  # he/she is really smiling
                        print("3")
                        if (flag == 1):
                            event_desc = '%s正在与义工交互' % (id_card_to_name[old_people_name[j_index]])
                            event_location = '房间桌子'
                            print('[EVENT] %s, 房间桌子, %s正在与义工交互.' % (current_time, id_card_to_name[old_people_name[j_index]]))
                            # sql语句
                            sql2 = (
                                        "insert into event_info(event_date,event_type,event_desc,event_location) value(now(),0,'%s正在与义工交互','房间桌子')" % (
